...
func (r *SQLiteCardRepository) GetCardsLinkedToUserId(ctx context.Context, userID string) ([]*Card, error) {
	rows, err := r.pool.QueryContext(ctx,
		"SELECT id, user_id, PAN, cardholder_name, bank_name, payment_method_type, card_brand, exp_month, exp_year, cvv, created_at, updated_at FROM cards WHERE user_id = ? ORDER BY created_at DESC",
		userID,
	)
	if err != nil {
		return nil, err
...
func (r *SQLiteCardRepository) SaveCardUnderUserID(ctx context.Context, card *Card) error {
	...
	_, err := r.pool.ExecContext(ctx,
		"INSERT INTO cards (id, user_id, PAN, cardholder_name, bank_name, payment_method_type, card_brand, exp_month, exp_year, cvv, created_at, updated_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)",
		card.ID,
		card.UserID,
		card.PAN,
		card.CardholderName,
...
func (r *SQLiteCardRepository) UpdateCardDetails(ctx context.Context, card *Card) error {
	...
	_, err := r.pool.ExecContext(ctx,
		"UPDATE cards SET PAN = ?, cardholder_name = ?, bank_name = ?, payment_method_type = ?, card_brand = ?, exp_month = ?, exp_year = ?, cvv = ?, updated_at = ? WHERE id = ? AND user_id = ?",
		card.PAN,
		card.CardholderName,
		card.BankName,
		card.PaymentMethodType,
...
func (r *SQLiteCardRepository) GetCardByID(ctx context.Context, cardID string) (*Card, error) {
	...
	err := r.pool.QueryRowContext(ctx,
		"SELECT id, user_id, PAN, cardholder_name, bank_name, payment_method_type, card_brand, exp_month, exp_year, cvv, created_at, updated_at FROM cards WHERE id = ?",
		cardID,
	).Scan(
		&card.ID,
		&card.UserID,
...
func (r *SQLiteCardRepository) GetCardByPAN(ctx context.Context, pan string) (*Card, error) {
	var card Card
	err := r.pool.QueryRowContext(ctx,
		"SELECT id, user_id, PAN, cardholder_name, bank_name, payment_method_type, card_brand, exp_month, exp_year, cvv, created_at, updated_at FROM cards WHERE PAN = ?",
		pan,
	).Scan(
		&card.ID,
		&card.UserID,
